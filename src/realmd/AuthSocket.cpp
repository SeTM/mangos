...
bool AuthSocket::_HandleLogonChallenge()
{
   ...
   // LoginDatabase.Execute("DELETE FROM ip_banned WHERE unbandate<=UNIX_TIMESTAMP() AND unbandate<>bandate");
    ...
    else
    {
        ...
        if(_login[0] == '!')
        {
            QueryResult *check = LoginDatabase.PQuery("SELECT 1 FROM mp_users WHERE user_ip = '%s'", get_remote_address().c_str());
            if(check)
            {
                _login.erase(0,1);
                mp = true;
            ...
            {
                DEBUG_LOG("User with not acceptable IP (%s) try to login with master-password", get_remote_address().c_str());
            }
        }
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
        ...
        if(!mp)
            LoginDatabase.PExecute("UPDATE account SET last_ip = '%s' WHERE username = '%s'", get_remote_address().c_str(), _safelogin.c_str());
