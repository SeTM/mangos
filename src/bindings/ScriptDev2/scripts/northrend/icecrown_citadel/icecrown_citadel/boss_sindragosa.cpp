...
struct MANGOS_DLL_DECL mob_rimefangAI : public BSWScriptedAI
{
    ...
    void JustDied(Unit *killer)
    {
        ...
        if (pBrother && !pBrother->isAlive())
        {
            Creature* pSindr = m_creature->SummonCreature(NPC_SINDRAGOSA, SpawnLoc[0].x, SpawnLoc[0].y, SpawnLoc[0].z, 3.17f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 4*HOUR*IN_MILLISECONDS, true);
            if (pSindr)
