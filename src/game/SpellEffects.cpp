    ...
    if( unitTarget && unitTarget->isAlive())
    {
        switch(m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                switch(m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    case 28836:
                    {
                        ...
                        case 28834:
                        case 28835:
                            if (Aura *mark = unitTarget->GetAura(m_triggeredByAuraSpell->Id, SpellEffectIndex(0)))
                            {
            ...
            case SPELLFAMILY_DRUID:
            {
                ...
                {
                    Aura *ImprovedAura =  m_caster->HasAura(57849) ? m_caster->GetAura(57849,SpellEffectIndex(0)) : m_caster->HasAura(57850) ?
                    m_caster->GetAura(57850,SpellEffectIndex(0)) : m_caster->HasAura(57851) ? m_caster->GetAura(57851,SpellEffectIndex(0)) : NULL;
            ...
            case SPELLFAMILY_GENERIC:
            {
                switch( m_spellInfo->Id )
                {
                    case 55647: // Aberrations
                    {
                        ...
                        MaNGOS::GameObjectListSearcher<MaNGOS::AnyGameObjectInPointRangeCheck> searcher(gobList, gobject_check);
...
void Spell::EffectHealPct(SpellEffectIndex /*eff_idx*/)
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        if (!caster)
            return;
        //Glyph of Rune Tap mustn`t heal deathknight for 10%
            ...
            if(holder->GetSpellProto()->Dispel == DISPEL_MAGIC)
            {
                ...
                if (!holder->IsPositive())
                    ...
                    positive = (holder->GetSpellProto()->AttributesEx & SPELL_ATTR_EX_NEGATIVE)==0 ||
                                holder->GetSpellProto()->SpellFamilyName == SPELLFAMILY_SHAMAN && // Elemental Oath
                                holder->GetSpellProto()->SpellIconID == 3053;
                ...
                case 64456:                                 // Feral Essence Application Removal
                    {
                        ...
                        unitTarget->RemoveAuraHolderFromStack(m_spellInfo->CalculateSimpleValue(eff_idx));
                        return;
                    }
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (m_spellInfo->Id == 54846)
            {
                if (Aura * aurEff = unitTarget->GetAura(SPELL_AURA_PERIODIC_DAMAGE,SPELLFAMILY_DRUID,0x00000002,0,m_caster->GetGUID()))
                {
                    ...
                    if (countMin < countMax)
                    {
                        aurEff->GetHolder()->SetAuraDuration(uint32(aurEff->GetAuraDuration()+3000));
                        aurEff->GetHolder()->SetAuraMaxDuration(countMin+3000);
...
void Spell::EffectStealBeneficialBuff(SpellEffectIndex eff_idx)
{
    ...
    if (!steal_list.empty())
    {
        ...
        for (int32 count=0; count < damage && list_size > 0; ++count)
        {
            ...
            // Try dispel
            if (roll_chance_i(miss_chance))
                fail_list.push_back(holder->GetId());
            else
