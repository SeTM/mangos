...
static uint32 copseReclaimDelay[MAX_DEATH_COUNT] = { 0, 30, 60, 120 };
...
void Player::RegenerateAll(uint32 diff)
{
    ...
    if (!isInCombat() || HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT) ||
        HasAuraType(SPELL_AURA_MOD_HEALTH_REGEN_IN_COMBAT) || IsPolymorphed() || m_bonusHealthRegen > 0 )
    {
...
void Player::RegenerateHealth(uint32 diff)
{
    ...
    addvalue += m_bonusHealthRegen * 0.4; // temp, need rewrite. with this ^
...
void Player::UpdateHonorFields()
{
    ...
    if (HonorKills >= 100 && HonorKills < 1000)
        victim_rank = 1;
    else if (HonorKills >= 1000 && HonorKills < 2000)
        victim_rank = 2;
    else if (HonorKills >= 2000 && HonorKills < 4650)
        victim_rank = 3;
    else if (HonorKills >= 4650 && HonorKills < 9500)
        victim_rank = 4;
    else if (HonorKills >= 9500 && HonorKills < 15350)
        victim_rank = 5;
    else if (HonorKills >= 15350 && HonorKills < 19200)
        victim_rank = 6;
    else if (HonorKills >= 19200 && HonorKills < 23050)
       victim_rank = 7;
    else if (HonorKills >= 23050 && HonorKills < 26900)
        victim_rank = 8;
    else if (HonorKills >= 26900 && HonorKills < 34600)
        victim_rank = 9;
    else if (HonorKills >= 34600 && HonorKills < 38450)
        victim_rank = 10;
    else if (HonorKills >= 38450 && HonorKills < 42300)
        victim_rank = 11;
    else if (HonorKills >= 42300 && HonorKills < 46150)
        victim_rank = 12;
    else if (HonorKills >= 46150 && HonorKills < 50000)
        victim_rank = 13;
...
Item* Player::StoreItem( ItemPosCountVec const& dest, Item* pItem, bool update )
{
    ...
    GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_OWN_ITEM, entry, pItem->GetCount());
    return lastItem;
}
...
void Player::ApplyEnchantment(Item *item, EnchantmentSlot slot, bool apply, bool apply_dur, bool ignore_condition)
{
    ...
    if (HasFlag(UNIT_FIELD_FLAGS,UNIT_FLAG_DISARMED) && apply)
        return;
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorGuid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if ((bag == NULL_BAG && slot == NULL_SLOT) || IsInventoryPos(bag, slot))
    {
		...
		if (pItem = StoreNewItem(dest, item, true))
			if (pItem->IsEligibleForRefund() && crItem->ExtendedCost )
				AddRefundable(pItem->GetGUID(), crItem->ExtendedCost);
	}
...
void Player::SendCorpseReclaimDelay(bool load)
{
    ...
    if (load)
    {
        ...
        delay = GetCorpseReclaimDelay(m_ExtraFlags & PLAYER_EXTRA_PVP_DEATH);

    if (delay)
    {
        m_corpseReclaimDelay = time(NULL)+delay;
...
uint32 Player::LookupRefundable(ObjectGuid itemGUID)
{
