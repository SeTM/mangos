    ...
    for(AchievementCriteriaEntryList::const_iterator itr = achievementCriteriaList.begin(); itr != achievementCriteriaList.end(); ++itr)
    {
        ...
        switch (type)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SPELL:
                ...
                if(achievementCriteria->ID!=exception && !GetPlayer()->HasSpell(achievementCriteria->learn_spell.spellID))
                    continue;
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_OWN_ITEM:
                ...
                if(miscvalue1 && achievementCriteria->own_item.itemID != miscvalue1)
                    continue;
                //change = GetPlayer()->GetItemCount(achievementCriteria->own_item.itemID, true);  wrong to gather achievements
                change = miscvalue2;
                progressType = PROGRESS_ACCUMULATE;
                break;
            case ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA:
...
uint32 AchievementMgr::GetCriteriaProgressMaxCounter(AchievementCriteriaEntry const* achievementCriteria)
{
    switch(achievementCriteria->requiredType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SPELL:
            return 1;
        case ACHIEVEMENT_CRITERIA_TYPE_HONORABLE_KILL:
            return achievementCriteria->honor_kill.count;
        case ACHIEVEMENT_CRITERIA_TYPE_OWN_ITEM:
...
void AchievementMgr::BuildAllDataPacket(WorldPacket *data)
{
    for(CompletedAchievementMap::const_iterator iter = m_completedAchievements.begin(); iter!=m_completedAchievements.end(); ++iter)
    {
        ...
        if(achiev->flags & ACHIEVEMENT_FLAG_COUNTER)
            continue;
