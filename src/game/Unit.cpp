        ...
        if (holder)
        {
            if (!(holder->IsPermanent() || holder->IsPassive()) )
            {
                ...
                for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
                {
                    if (Aura *aura = holder->GetAuraByEffectIndex(SpellEffectIndex(i)))
                    {
                        if (aura->GetAuraDuration() > 0 && aura->GetAuraDuration() < duration)
                        {
                            RemoveSingleAuraFromSpellAuraHolder(holder, aura->GetEffIndex(), AURA_REMOVE_BY_EXPIRE);
                            removedAura = true;
                            break;
...
{
    if(!HasAuraState(flag, spellProto, ObjectAccessor::GetUnit(*this, caster)))
        return false;
...
Pet* Unit::GetPet() const
{
    ...
    if (!pet_guid.IsEmpty())
    {
        Map *m = GetMapWithoutAssert();
        if(m)
            if(Pet* pet = m->GetPet(pet_guid))
                return pet;
...
uint32 Unit::SpellDamageBonusDone(Unit *pVictim, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    switch(spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            // Glyph of Smite
            else if (spellProto->SpellFamilyFlags & UI64LIT(0x00000080))
            {
                // Holy Fire
        ...
        {
            {
              ...
              for(AuraList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
              {
                  if ((*i)->GetModifier()->m_miscvalue == 7282)
                  {
                      pVictim->RemoveAuraHolderFromStack(50510);
                      switch((*i)->GetSpellProto()->Id)
                      {
...
{
    if(!HasAuraType(SPELL_AURA_ABILITY_IGNORE_AURASTATE))
        return false;

    if(spell->SpellFamilyName == SPELLFAMILY_MAGE)
        return true; 

    Unit::AuraList const& stateAuras = GetAurasByType(SPELL_AURA_ABILITY_IGNORE_AURASTATE);
