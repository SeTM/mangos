...
m_isPersistent(false), m_in_use(0), m_spellAuraHolder(holder)
{
    ...
    if (sSpellMgr.GetSpellElixirSpecific(spellproto->Id))                   // table spell_elixer have it all
        if (target->HasAura(53042) && target->HasSpell(spellproto->EffectTriggerSpell[0]))
        {
            GetHolder()->SetAuraMaxDuration(GetAuraMaxDuration() * 2);
            GetHolder()->RefreshHolder();
            GetModifier()->m_amount *= 1.3f;
        }
...
void AreaAura::Update(uint32 diff)
{
    ...
    else                                                    // aura at non-caster
    {
        ...
        if( !caster || caster->hasUnitState(UNIT_STAT_ISOLATED) ||
            ...
            target->IsImmuneToSpell(GetSpellProto()))
        {
            ...
            switch(GetId())
            {
                ...
                case 43115: // Q: Test at Sea
                    if( apply && target->GetEntry() == 24120 )
                    {
                        switch( irand(0, 3))
                        {
                            case 0: target->MonsterYell("I don't feel so good...", LANG_UNIVERSAL); break;
                            case 1: target->MonsterYell("That liquid... it reeks!", LANG_UNIVERSAL); break;
                            case 2: target->MonsterYell("Someone shoot that bat down!", LANG_UNIVERSAL); break;
                        }
                        if (Unit* caster = GetCaster())
                            if(caster->GetTypeId() == TYPEID_PLAYER )
...
void Aura::HandleAuraMounted(bool apply, bool Real)
{
    ...
    if(apply)
    {
        ...
        if (GetSpellProto()->Id == 66090 && team == HORDE) 
        {
...
void Aura::HandleAbilityIgnoreAurastate(bool apply, bool Real)
{
    ...
    if(apply)
        switch (GetId())
    {
		case 44544:	GetHolder()->SetAuraCharges(2);	break;
		case 52437:	GetHolder()->SetAuraCharges(1);	break;
...
void Aura::HandleAuraInitializeImages( bool Apply, bool Real)
{
	...
	else
	{
		...
		{
			for (uint8 i = 0; i < 3; ++i)
				target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i, 0);
		}
		else // TYPEID_UNIT
		{
			((Creature*)target)->LoadEquipment(((Creature*)target)->GetEquipmentId());
		}
	}
}
